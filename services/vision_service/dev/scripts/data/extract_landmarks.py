"""
Script para extraer landmarks de imagenes usando MediaPipe.

Uso:
    python extract_landmarks.py

Procesa las imagenes de train/ y test/, extrae landmarks 
con MediaPipe y guarda en formato .npz (NumPy comprimido).

Recursos: ~500MB RAM, ~30-60 min para 83K imagenes
"""
...
class LandmarkExtractor:
    ...
    def process_split(self, split: str = 'train') -> tuple:
        ...
        for folder in letter_folders:
            ...
            print(f"   OK: {len(letter_landmarks)} | Fallidas: {len(images) - len(letter_landmarks)}")
...
def main():
    ...
    try:
        ...
        if X_train is not None:
            ...
            # Guardar
            np.savez_compressed(
                OUTPUT_PATH / 'landmarks_train.npz',
                X=X_train,
                y=y_train
            )
            print(f"[OK] Guardado: landmarks_train.npz")
        ...
        if X_test is not None:
            ...
            np.savez_compressed(
                OUTPUT_PATH / 'landmarks_test.npz',
                X=X_test,
                y=y_test
            )
            print(f"[OK] Guardado: landmarks_test.npz")
        
        # Guardar mapeo de etiquetas
        with open(OUTPUT_PATH / 'label_encoder.pkl', 'wb') as f:
            pickle.dump(label_map, f)
        print(f"[OK] Guardado: label_encoder.pkl")
        ...
        print(f"Tasa exito: {extractor.processed / (extractor.processed + extractor.failed) * 100:.1f}%")
