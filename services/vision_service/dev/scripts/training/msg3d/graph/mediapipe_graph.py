...
def hand_edges(offset):
    ...
    return [
        ...
        (offset + 0, offset + 5),   # wrist -> index base
        (offset + 0, offset + 9),   # wrist -> middle base
        (offset + 0, offset + 13),  # wrist -> ring base
        (offset + 0, offset + 17),  # wrist -> pinky base
...
def get_edge_list():
    """Retorna la lista de edges para visualización."""
    return ALL_EDGES


def visualize_graph():
    """Imprime información del grafo para debugging."""
    print("=" * 60)
    print("MediaPipe Skeleton Graph")
    print("=" * 60)
    print(f"Total nodes: {NUM_NODES}")
    print(f"Total edges: {NUM_EDGES}")
    print(f"\nPose landmarks: 0-32 ({len(POSE_EDGES)} edges)")
    print(f"Left hand: 33-53 ({len(LEFT_HAND_EDGES)} edges)")
    print(f"Right hand: 54-74 ({len(RIGHT_HAND_EDGES)} edges)")
    
    A = get_adjacency_matrix()
    print(f"\nAdjacency matrix shape: {A.shape}")
    print(f"Nonzero entries: {np.count_nonzero(A)}")
    print(f"Sparsity: {1 - np.count_nonzero(A) / A.size:.2%}")
    
    # Degree distribution
    degrees = np.sum(A > 0, axis=1) - 1  # -1 for self-loop
    print(f"\nDegree stats: min={degrees.min()}, max={degrees.max()}, mean={degrees.mean():.1f}")
