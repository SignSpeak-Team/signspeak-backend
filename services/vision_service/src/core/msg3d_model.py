...
from .msg3d_graph import get_adjacency_matrix
...
class TemporalConv(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=9, stride=1, residual=True
    ):
        super().__init__()
...
class STGCNBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A, stride=1, dropout=0.25):
        ...
        self.tcn = TemporalConv(
            out_channels, out_channels, kernel_size=9, stride=stride
        )
...
class MSG3D(nn.Module):
    ...
    def __init__(
        ...
        A = np.stack(
            [np.eye(num_point, dtype=np.float32), A_spatial, A_spatial], axis=0
        )
